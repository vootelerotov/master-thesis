...
\tosup{\sout{About programs that do not compile, a more important category are open programs, such as modules and device drivers, that do not compile alone, but can be analyzed without the rest of the system, so not just considering all runs, but under all possible behaviors of the environment.}}
...
\tosup{\sout{You will need to check the typography at some point, $if$ is definitely not what you want here.}}
\todisc{Wrapped inline code with macro, defined as \\texttt  as of now, just like for code in CFG  and in equations. So code is either in listing or in \\texttt.}
...
To be able to formalize concrete semantics, we first must define what we mean under program. In this section we will presume that programs are represented as flow graphs.
The flow graphs serve as common intermediate representation that is powerful enough that we can express all programs written in higher level languages and at same time,
considerable easier to mathematically reason about.
\tosup{\sout{I'd rather say that we work on a common intermediate representation (IR), the control flow graph, or programs are represented as flow graphs, rather than this being the definition of a program.}}
...
\tosup{\sout{Whoa, $s$ is used for everything!}}
\todisc{Typo.}
...
\end{defin}
\tosup{\sout{Make sure you are consistent with $p$ versus $proc$; it seems entirely random above.}}
\todisc{Made $p$ to signify process everywhere and $f()$ a process call.}
\tosup{\sout{Why are there separate Pos and Neg rules?? This defeats the purpose of having a separate intra-proc semantics. Can't you just make a transition whenever the intra one can do it?}}
\todisc{Indeed.}
...
\tosup{\sout{Might want to anticipate future developments: ``we will first lift our inter-procedural evaluation relation to a semantics of executing individual threads, which supports the spawning of other threads. We will later use this to define a semantics that takes thread interleavings into account.''}}
\todisc{Introduction of a list that nothing is done with did not seem obvious, like this one very much.}
...
\begin{defin}
  ...
  \tosup{\sout{I injected $t_i$ above, but you should inject it below as well. Question: why are the spawned threads $p$-s with a star. That would seem like they are a list or something? I'm also wondering if maybe square brackets would be better for lists to make clearer that you have singleton call stacks.}}
  \todisc{I think the idea behind the star was to draw attention to them, to make sure that one picks up that those are singleton lists. Stacks rewritten using square brackets.}
\end{defin}
...
\end{defin}
\tosup{\sout{Try to avoid notation like $\sqsubseteq \subseteq D \times D$. I think you can just assume people know what a relation is, and if they don't, this is even harder for them to parse.}}
\todisc{Changed, also changed $\subseteq X \times Y$ to ``relation between $X$ and $Y$'' in previous definitions. Teached DME for too many years I guess :)}
...
\tosup{\sout{Again, this $\sqsubseteq = \lb \lp false, true \rp \rb$ looks weird just say ``with the ordering $\mathit{false} \sqsubseteq \mathit{true}$''.}}
...
\tosup{\sout{Why does $D$ need to be finite?}}
\todisc{Good question.}
\tosup{\sout{I don't understand what is meant with ``while describing that specific state for every state in our program''.}}
\todisc{Rewrote}
...
\toadd{Wrap words in math mode with a macro}
...
\toans{Yes, it is the precise relation between these things, see ``Duality (order theory)'' on Wikipedia, but the question is if your intended audience gets it,  and maybe you could have introduced the concept earlier and avoided repetition. All extremely low-priority: things are correct as is.}

\toguide{So, why bother with the lattice, what does it give us?}

One might still be wondering, why do we want to use lattices here, instead of simply using sets without the partial order or the lattice operations as abstract domains. Consider following program.
...
\begin{equation*}
\lllb \mcode{x = x + 1} \rrrb_{Stmt}^{\absint} \left( d \right) =
\begin{cases}
...
We demand that the functions $\lllb s \rrrb_{Stmt}^{\absint}$ are consistent to functions $\lllb s \rrrb_{Stmt}$, that is, for each $s \in S, d \in D$ if $s \descriptor d$ then $ \lllb s \rrrb_{Stmt} \descriptor \lllb d \rrrb_{Stmt}^{\absint}$. This relation is illustrated in the following figure.
\tosup{We also demand that all figures are mentioned in the text!}
\todisc{Fair demand, I am guessing that this does not mean that it has to be mentioned by name, ``following figrue'' is suitable?}
...
\toans{Yes, many things could be included, but here I would probably just state that instead of considering all paths and joining at the very end, we will consider the data flow and join information as soon as different paths converge, thereby computing a single domain value for each location. Then say, that it can be shown and cite the Nielsen textbook that this system will over-approximate the MOP solution.}
\toadd{Do the above}
...
\toans{This seems at least fine for now. Whether threads are added depends on if it is actually needed for subsequent chapters.}
