...
The type \inlinecode{ctx} with type parameters $\lp D.t, G.t \rp$ encapsulates the both the local and global state, offering access to helper functions on them. 
...
The \inlinecode{query} function enables communication between different analyses that have have been combined into one and are being performed at the same time -- 
...
The \inlinecode{special} function handles functions for which the source code is not available for Goblint to analyze. In mutex analyses,
this function modifies the abstract state -- functions calls to functions such as \inlinecode{\_\_raw\_lock\_unlock} and \inlinecode{\_lock\_kernel} 
...
As part of this thesis, the region analyses was extended to include the time dimension in addition.

A new concurrency domain was added that improves the way Goblint handles detection of processes that are guaranteed to only run in one thread at a time.
A way to append the time partition information to the left side of the access was added with a way to transform the time partition that can be used inside transfer functions. 
...
one of the oldest analysis in Goblint and Goblint does not have a preproccessing phase, then the complexity added to the already complex base analysis is quite extensive.
...
We divide the potential data races into two categories. We say that the potential race is \textit{direct} if Goblint deems it unsafe based on direct access present in the source code
Goblint is analysing. For an example, the direct access could be a variable assignment.
An \textit{indirect race} in reported when there is a possibility of an race at a location when one considers functions for what Goblint does not have source code for.
This can happen if driver exposes some of its state to using a function defined in Linux kernel.   

In the Figure \ref{evaluation} the columns denoted as \textit{Region} contain results of analysis of the benchmark suite performed by Goblint with the space based region analysis active while the column denoted as \textit{Enhanched Region} are results of achieved by additionally adding the time partitioning. The numbers indicate real data races or false-positives found by Goblint.

As seen by the results of the benchmark, the effect of time partition added to the region analyses has depends heavily on the driver. In some drivers it enables Goblint to deem quite a lot of previously unsafe locations to be safe, lessening the amount of false-positives.
