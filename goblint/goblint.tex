...
\toadd{Cite the arctile written, \sout{doctorial thesis of Kalmer and Vesal, github repo}}
...
Goblint is a constraint system based analysis tool, that performs abstract interpretation, building on top of the theorectical ideas described in previous sections. Goblint is written in OCaml and is publicly available on Github.
...
A way to append the time partition information to the left side of the access was added with a way to transform the time partition that can be used inside tranfer functions. 

The implementation makes vast use of the \inlinecode{part\_access} method. A remaining technical difficulty is that the concurency domain is ingrained in the base analysis,
one of the oldest analysis in Goblint and Goblint does not have a preproccessing phase, then the complexity added to the already compelx base analysis is quite extensive.
...
Evaluating the effect of the enhanched region analysis on the set of character devices used as benchmark suit for Goblint gave following result.

\toadd{Actual benchmark}
\toadd{Table, comparision between fork and the mainline for each driver. Similar effect to Kalmers much more elegant implementation.}
