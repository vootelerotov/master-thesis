...
echo hi > my-driver
\end{lstlisting}
then file is opened, written into and then closed.
...
Device drivers also expose \textit{init} and \textit{exit}, which kernel can use to register and de-register the device driver. In case of \textit{Character}, registering the device initializes the counter $i$ to zero and then registers the \textit{file operations}. 
