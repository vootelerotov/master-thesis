...
\kalmer{"inevitable loss of precision is inevitable" --> peaksid ütlema midagi sellist, et tuleb valida sobiv abstraktne domeen või jälgida sobivaid omadusi. Muidu see on ok.}
...
\kalmer{"kui meil õnnestub eraldada mälu erinevateks regioonideks, siis võib iga regioooni kaitsta eri lukustus-mehhanismiga", näiteks räsitabelid}

Consider a hash table using chaining to handle hash collisions that has a lock for each of the buckets.

\kalmer{joonista lukud ka pildile}
...
\kalmer{see on õige, aga lukk võetakse räsi järgi. õige negatiivne näide on siis, kui räsitabelis on salvestatud viit jagatud muutujale (mis on siis mitmes regioonis korraga)}
...
\kalmer{jälle, sa ei tohi unustada lukke --- ilma lukuta race-b sama lõim iseendaag}

In case of the hash table, the most obvious partition for the regions would be to separate the buckets of the hash table.

\kalmer{jah aga seda sa ei saa vabalt valida: kui jagatud viit salvestatakse räsitabelisse siis ei saa jagada niimoodi eraldi regioonideks}
...
\kalmer{mina soovitan alati kasutada ``columns=fullflexible''}
...
\kalmer{palun kasuta siin mingit näidise-environment-i ... väga raske on jälgida kuna näide ära lõppeb ja kuna uus algab.}

As mentioned previously, \inlinecode{init} and \inlinecode{exit} functions are used by kernel to register and deregister a device.
...
\kalmer{sa mõtled siin ikka \texttt{*p} ja \texttt{*q} ---  deklareerides alati kirjuta * muutuja külge, ehk \texttt{*p} tüüp on \texttt{int} siis jääb see lihtsalt meelde}
...
\kalmer{Siin on kõik segamini nagu puder ja kapsad. Tee eraldi näited ja ära unusta lukke. mina soovitaks isegi teha nagu siin .tex kommentaaris
% void close()
% {
%   lock(\&close\_lock);
%   *p = *q - 1;
%   unlock(\&release\_lock);
% }
% void open()
% {
%   lock(\&open\_lock);
...
\kalmer{Sa pead nendele hulkadele mingi interpretatsiooni andma. $\{\{\alpha,\beta\},\{\gamma\}\}$ on $(\alpha\land{}\beta)\lor{}\gamma$ kus $A \implies \neg{}B$ ja $B\implies\neg{A}$.}
