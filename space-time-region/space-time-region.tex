...
This does allow to eliminate the data race between the assignments to \inlinecode{i} in \inlinecode{init} and \inlinecode{exit} functions.
...
If we are able to separate the memory region only containing \inlinecode{file} then we can partition this further by dividing the time dimension to $default, file\_open$ and $file\_release$,
