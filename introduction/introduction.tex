...
The aim of this thesis is to increase the precision of Goblint by eliminating one type of false-positives found while manually reviewing the results as part of benchmarking done in \toadd{Article}.
...
are called. This, combined with that device drivers are written in subset of C, a low level language with very few built in concurency abstractions, means that debugging the concurrency issues in device drivers is notoriously difficult. In addition to that, the bugs in device drivers have a big impact and can be attributed for a large share of system crashes. 
