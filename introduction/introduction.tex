...
are called. This, combined with that device drivers are written in subset of C, a low level language with very few built in concurency abstractions,
means that debugging the concurrency issues in device drivers is notoriously difficult.
In addition to that, the bugs in device drivers have a big impact and can be attributed for a large share of system crashes. 
...
For an example, let there be a an assignment to memory location \inlinecode{i} in both functions \inlinecode{init} an \inlincecode{exit}. 
If we know that both functions are called only once and that call to \inlinecode{init} must complete before \inline{exit} can be called thet we can safely rule out a data race on \inlincecode{i}.
